		...
		if (label.equalsIgnoreCase("drop")) {
			...
			if (args.length == 0) {
				if (p.hasPermission("dropparty.drop") || p.hasPermission("dropparty.*")) {
					...
					DropGUI.fillDropGUi();
					DropGUI.dropGUI.setItem(26, tokens);
						...
						if (args[1].equalsIgnoreCase("add")) {
							if (Integer.parseInt(args[2]) >= 1) {
								...
								if (DropParty.dropLocations.containsKey(selectedIndex)) {
									...
									Main.getPlugin(Main.class).dropLocsConfig.set("Drop Locations." + selectedIndex + ". Y.", p.getLocation().getX());
									Main.getPlugin(Main.class).dropLocsConfig.set("Drop Locations." + selectedIndex + ". Z.", p.getLocation().getX());
									Main.getPlugin(Main.class).dropLocsConfig.set("Drop Location." + selectedIndex + ". World.", p.getWorld().getName());
